...
class Printing {
   ...
   */
  text(value) {
    this._queue([_constants.PRINTING_COMMANDS.COMMAND_ADD_TEXT, [value]]);
    return this;
  }
  ...
  textLine(charsPerLine, params) {
    const text = (0, _utils.spaceBetween)(Math.ceil(charsPerLine / this._currentFontWidth), params);
   ...
   */
  newline(value) {
    this._queue([_constants.PRINTING_COMMANDS.COMMAND_ADD_NEW_LINE, [value || 1]]);
    return this;
  }
  ...
  underline(value) {
    ...
    this._queue([_constants.PRINTING_COMMANDS.COMMAND_ADD_TEXT_STYLE, [this._convertToEposBool(this._state.underline), this._convertToEposBool(this._state.bold)]]);
    return this;
  }
  ...
  _convertToEposBool(value) {
    const res = value ? _constants.EPOS_BOOLEANS.EPOS2_TRUE : _constants.EPOS_BOOLEANS.EPOS2_FALSE;
    return res;
  }
  ...
  smooth(value) {
    ...
    this._queue([_constants.PRINTING_COMMANDS.COMMAND_ADD_TEXT_SMOOTH, [this._convertToEposBool(this._state.smooth)]]);
    return this;
  }
  ...
  size(height, width) {
    ...
    this._queue([_constants.PRINTING_COMMANDS.COMMAND_ADD_TEXT_SIZE, [height || 1, width || 1]]);
    return this;
  }
  ...
  align(value) {
    ...
    this._queue([_constants.PRINTING_COMMANDS.COMMAND_ADD_ALIGN, [_constants.PRINTING_ALIGNMENT[value]]]);
    return this;
  }
  image(imageSource, {
    width,
    ...
    (0, _utils.assertNativeCommands)([color, mode, halftone], 'image');
    (0, _utils.assertImageSource)(imageSource);
    ...
    const image = _reactNative.Image.resolveAssetSource(imageSource);
    this._queue([_constants.PRINTING_COMMANDS.COMMAND_ADD_IMAGE, [image, width, (0, _utils.getNativeCommand)(color), (0, _utils.getNativeCommand)(mode), (0, _utils.getNativeCommand)(halftone), brightness]]);
    return this;
  }
  ...
  imageBase64(image, width) {
    ...
    this._queue([_constants.PRINTING_COMMANDS.COMMAND_ADD_IMAGE_BASE_64, [image, width]]);
    return this;
  }
  ...
  imageAsset(image, width) {
    ...
    this._queue([_constants.PRINTING_COMMANDS.COMMAND_ADD_IMAGE_ASSET, [image, width]]);
    return this;
  }
  ...
  barcode({
    ...
    if (!(typeof _constants.BARCODE_TYPE[type] === 'number')) {
    ...
    this._queue([_constants.PRINTING_COMMANDS.COMMAND_ADD_BARCODE, [value, _constants.BARCODE_TYPE[type], _constants.BARCODE_HRI[hri], width, height]]);
    return this;
  }
  ...
  qrcode({
    ...
    if (!(typeof _constants.QRCODE_TYPE[type] === 'number')) {
      if (_reactNative.Platform.OS === 'ios' && type === 'EPOS2_SYMBOL_QRCODE_MICRO') {
        throw new Error('QRCODE_MICRO is not supported on iOS');
      } else {
        throw new Error('Unknown type of QR Code');
      }
    }
    if (!(typeof _constants.QRCODE_LEVEL[level] === 'number')) {
    ...
    this._queue([_constants.PRINTING_COMMANDS.COMMAND_ADD_QRCODE, [value, _constants.QRCODE_TYPE[type], _constants.QRCODE_LEVEL[level], width]]);
    return this;
  }
   ...
   */
  cut() {
    this._queue([_constants.PRINTING_COMMANDS.COMMAND_ADD_CUT, []]);
    return this;
  }
   ...
   */
  data(uint8Array) {
    const buffer = new _utils.BufferHelper();
    const base64String = buffer.bytesToString(uint8Array, 'base64');
    this._queue([_constants.PRINTING_COMMANDS.COMMAND_ADD_DATA, [base64String]]);
    return this;
  }
  ...
  addPulse(drawerKickConnector = 'EPOS2_DRAWER_2PIN') {
    (0, _utils.assertNativeCommands)([drawerKickConnector], 'addPulse');
    this._queue([_constants.PRINTING_COMMANDS.COMMAND_ADD_PULSE, [(0, _utils.getNativeCommand)(drawerKickConnector)]]);
    return this;
  }
  send(params) {
    const timeout = params?.timeout;
    if (timeout) {
      if (!Number.isInteger(timeout) || timeout < _constants.PRINT_TIMEOUT_MIN || timeout > _constants.PRINT_TIMEOUT_MAX) {
        throw new Error(`Timeout should be an integer from ${_constants.PRINT_TIMEOUT_MIN} to ${_constants.PRINT_TIMEOUT_MAX}`);
      }
    }
    return this._send(this._buffer, params);
  }
}
var _default = exports.default = Printing;
